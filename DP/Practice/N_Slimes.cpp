...
pll fun(ll i1, ll i2){
    ...
    if(i1 == i2){
        return dp[i1][i2] = {0, v[i1]};
    }
    if(dp[i1][i2] != mp(-1LL, -1LL)){
        return dp[i1][i2];
    }
    ...
    fls(i, i1, i2 - 1){
        auto left = fun(i1, i);
        auto right = fun(i + 1, i2);
        ll cost = left.ff + right.ff + left.ss + right.ss;
        ll cur = left.ss + right.ss;
