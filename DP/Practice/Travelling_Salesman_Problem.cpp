...
void solve()
{
    ...
    n++;
    debug(n);
    vector<vll> adj(n, vll(n)), dp(n + 1, vll((1 << n) + 1, INF));
    fl(i, n){
        fl(j, n){
            if(i == j) continue;
            ll x;
    ...
    fl(mask, (1LL << n)){
        ...
        fl(i, n){
            ...
            fls(j, 1, n - 1){
                if(!(mask & (1 << j)) or i == j or dp[i][mask ^ (1 << j)] == INF) continue;
                debug(j, i, mask, mask ^ (1 << j));
                dp[j][mask] = min(dp[j][mask], dp[i][mask ^ (1 << j)] + adj[i][j]);
            }
            debug(mask, i, dp[i][mask]);
        }
    }
    ll ans = INF;
    fls(i, 1, n - 1){
