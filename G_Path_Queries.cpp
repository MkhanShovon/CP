...
public:
    ...
    void unite(ll u, ll v) {
        ...
        if (rootU != rootV) {
            ...
            ll a=size[rootU], b=size[rootV];
            pairs-=(a*(a-1)/2+b*(b-1)/2);
            size[rootU] += size[rootV];
            pairs+=(size[rootU]*(size[rootU]-1)/2);
...
void solve()
{
    ...
    ll iq = 0, ie = 0;
    vll ans(q);
    while(iq < q and ie < n-1)
    {
        ll x = qr[iq].ff;
        ll i = qr[iq].ss;
        while(ie < n-1 and edges[ie].ff <= x)
        {
            dsu.unite(edges[ie].ss.ff, edges[ie].ss.ss);
            ie++;
        }
        ans[i] = dsu.get();
        iq++;
