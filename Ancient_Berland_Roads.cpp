...
{
private:
    vector<ll> parent, size, minElement, maxElement, sum;
    ll n;
    ll num, largest;
    mll mxSum;
...
public:
    ...
    void unite(ll u, ll v)
    {
        ...
        if (rootU != rootV)
        {
            ...
            mxSum[sum[rootV]]--;
            if(mxSum[sum[rootV]] == 0){
                mxSum.erase(sum[rootV]);
            }
            mxSum[sum[rootU]]++;
    ...
    ll getMx(){
        return mxSum.rbegin()->ff;
    }

    void update(ll u, ll val){
        ll rootU = find(u);
        mxSum[sum[rootU]]--;
        if(mxSum[sum[rootU]] == 0){
            mxSum.erase(sum[rootU]);
        }
        sum[rootU] -= sum[u];
        sum[u] = val;
        sum[rootU] += sum[u];
