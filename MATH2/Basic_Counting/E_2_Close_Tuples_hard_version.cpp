...
void solve()
{
    ...
    fl(i, len(a)){
        ll ub = upper_bound(vr(a), a[i] + k) - a.begin();
        if(ub < i){
            continue;
        }
        ll dif = pf[ub] - pf[i + 1];
        ll lim = min(m, cnt[a[i]]);
        fls(j, 1, lim){
            ans = (ans + (choose(cnt[a[i]], j) * choose(dif, m - j)) % MOD) % MOD;
            // debug(i, j, dif, m - j, choose(dif, m - j), ans);
        }
    }
