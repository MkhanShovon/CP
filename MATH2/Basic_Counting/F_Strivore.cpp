...
void solve()
{
    ...
    long long ans = 0;
    // Precompute powers if you like, or compute on the fly
    // We'll compute 25^i incrementally and 26^(K-i) by precomputing 26^K and multiplying by inv26 each step.

    long long pow26K = exp(26, K, MOD);
    long long inv26 = exp(26, MOD - 2, MOD);
    long long cur25 = 1;      // 25^0
    long long cur26 = pow26K; // 26^(K-0)

    for (long long i = 0; i <= K; ++i)
    {
        // term = C(n-1 + i, n-1) * 25^i * 26^(K-i)
        long long ways = choose((n - 1) + i, (n - 1));
        long long term = ways % MOD;
        term = (term * cur25) % MOD;
        term = (term * cur26) % MOD;
        ans = (ans + term) % MOD;
        ...
        // advance: i -> i+1
        cur25 = (cur25 * 25) % MOD;
        cur26 = (cur26 * inv26) % MOD; // divide by 26 each step
    }
