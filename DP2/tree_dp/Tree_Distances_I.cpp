...
// DFS function to calculate distances from source node
void dfs(ll u, ll parent, vector<vector<ll>>& adj, vector<ll>& dist) {
    for(ll v : adj[u]) {
        if(v != parent) {
            dist[v] = dist[u] + 1;
            dfs(v, u, adj, dist);
...
void solve()
{
    ...
    vll dist1 = dist;
    fill(vr(dist1), 0);
    dfs(farthest, 0, adj, dist1);
    
    ll far = 1;
    for(ll i = 1; i <= n; i++) {
        if(dist1[i] > dist1[far]) {
            far = i;
        }
    }
    fill(vr(dist), 0);
